...
{
    if ((((uint32_t)1U) << LLWU_SW_GPIO_PIN) & PORT_GetPinsInterruptFlags(LLWU_SW_PORT))
    {
        /* Disable interrupt. */
        PORT_SetPinInterruptConfig(LLWU_SW_PORT, LLWU_SW_GPIO_PIN, kPORT_InterruptOrDMADisabled);
        PORT_ClearPinsInterruptFlags(LLWU_SW_PORT, (((uint32_t)1U) << LLWU_SW_GPIO_PIN));
        g_UsbDeviceHidMouse.selfWakeup = 1U;
        /* Clear the event pending flag */
        EVENT_UNIT->EVTPENDCLEAR = (((uint32_t)1U) << LLWU_SW_IRQ);
    }
}
void SW_IntControl(uint8_t enable)
{
...
{
    USB_TimerInit(1, 1000U, CLOCK_GetFreq(kCLOCK_BusClk), HW_TimerCallback);
}
void HW_TimerControl(uint8_t enable)
{
...
void USB_PreLowpowerMode(void)
{
    ...
    /* Clear the event pending flag */
    EVENT_UNIT->EVTPENDCLEAR = (((uint32_t)1U) << irqNumber);
}
uint8_t USB_EnterLowpowerMode(void)
{
...
/* Install isr, set priority, and enable IRQ. */
    ...
    /* For RISCV, the event enable bit in event unit should be set for wakeup. */
    EVENT_UNIT->EVENTEN |= (((uint32_t)1U) << irqNumber);
}
#if USB_DEVICE_CONFIG_USE_TASK
void USB_DeviceTaskFn(void *deviceHandle)
{
...
#endif
{
    ...
    INTMUX_Init(INTMUX0);
#endif
    INTMUX_EnableInterrupt(INTMUX0, 0, LPIT1_IRQn);
