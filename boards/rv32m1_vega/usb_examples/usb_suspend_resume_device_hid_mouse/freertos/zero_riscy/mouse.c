...
void SW_Init(void)
{
    INTMUX_EnableInterrupt(INTMUX1, 0, LLWU_SW_IRQ);
    /* For RISCV, the event enable bit in event unit should be set for wakeup. */
    EVENT_UNIT->EVENTEN |= (((uint32_t)0xFFU) << INTMUX1_0_IRQn);
}
char *SW_GetName(void)
{
    return LLWU_SW_NAME;
...
void USB_PreLowpowerMode(void)
{
    ...
    EVENT_UNIT->EVTPENDCLEAR |= (((uint32_t)0xFFU) << INTMUX1_0_IRQn);
...
#endif
    CLOCK_EnableClock(kCLOCK_Lpit0);
    INTMUX_EnableInterrupt(INTMUX1, 0, LPIT0_IRQn);
    /* Set the LLWU pin */
